...
public class Menu {
	       ...
	       public static JButton sauv= new JButton(LoadImage.transformeb(100, 25, "/sauvegarde.jpg"));
	//Creation des menu
			...
			private static  JMenu menu4 = new  JMenu("Base des données");
	...
	//Creation des sous menu
			...
			private static JMenuItem menu4_2= new JMenuItem("Restauration");
	...
	public static void ajouter(){
		...
		sauv.setPreferredSize(new Dimension(100,25));
	...
	//Creation des racourci sur les menus / touche ALT+ Lettre entre cote' ';
		...
		menu3.setMnemonic('A');
   		...
   		menu4_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				JBackupController.executeCommand("matiere_maths", "postgres", "restore");
				JBackupController.executeCommand("utilisateur_mats", "postgres", "restore");
				JBackupController.executeCommand("droit_maths", "postgres", "restore");
				JBackupController.executeCommand("note_"+((String) LoginPrincipale.typeAn.getSelectedItem()).substring(0,4)+"_"
				+((String) LoginPrincipale.typeAn.getSelectedItem()).substring(5,9)+"_maths", "postgres", "restore");
				
				JBackupController.executeCommand("anne_"+((String) LoginPrincipale.typeAn.getSelectedItem()).substring(0,4)+"_"
				+((String) LoginPrincipale.typeAn.getSelectedItem()).substring(5,9)+"_maths", "postgres", "restore");
   	...
   	sauv.addActionListener(new ActionListener() {
		public void actionPerformed(ActionEvent arg0) {
				Main.main.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
				//Methode.sauvegarde_sems();
				JBackupController.executeCommand("utilisateur_mats", "postgres", "backup");
				JBackupController.executeCommand("droit_maths", "postgres", "backup");
				JBackupController.executeCommand("note_"+((String) LoginPrincipale.typeAn.getSelectedItem()).substring(0,4)+"_"
				+((String) LoginPrincipale.typeAn.getSelectedItem()).substring(5,9)+"_maths", "postgres", "backup");
				JBackupController.executeCommand("anne_"+((String) LoginPrincipale.typeAn.getSelectedItem()).substring(0,4)+"_"
				+((String) LoginPrincipale.typeAn.getSelectedItem()).substring(5,9)+"_maths", "postgres", "backup");
				JBackupController.executeCommand("matier_maths", "postgres", "backup");
				try {
				Methode.sauvegarde();
				} catch (IOException e) {
