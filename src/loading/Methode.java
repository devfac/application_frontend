...
return vide;
 ...
 public static boolean valideEns1(String mention,JTextField  numTxt,
		 JTextField nomTxt,JTextField prenomTxt) {
	 boolean val=false;

	 if(numTxt.getText().length()!=0) {
		 if(nomTxt.getText().length()!=0) {
				 val=true;
		 }else JOptionPane.showMessageDialog(null, " remplir le nom ", 
														 "ERREUR",JOptionPane.ERROR_MESSAGE);
		 }else JOptionPane.showMessageDialog(null, "veuiller remplir le numero d'inscription", 
													 "ERREUR",JOptionPane.ERROR_MESSAGE);
 ...
 public static void renitialDonner(JTextField txtTab1[],JPasswordField passTab1[],JButton mod,JButton supp,JButton enregmod,JButton enreg) {
 ...
 public static String dateTextImport(String date) {
	 String dtext="invalide date";
	 String mois;
	 int maxJour;
	 int maxMois;
	 ...
	 if(!date.equals("__/__/____")) {
		 ...
		 joursI=Integer.parseInt(date.substring(0,2));
		  moisI=Integer.parseInt(date.substring(3,5));
		  anneI=Integer.parseInt(date.substring(6,10));
		 if(moisI<8) {
			 if(moisI%2!=0)
				 maxJour=31;
			 else {
 ...
 public static void renitial1(JTextField tabField[],JTextField num,JComboBox<String> typeEt
		 ,JButton mod,JButton supp,JButton enregmod,JButton enreg,JTable table[],JCheckBox S[],JLabel image,JCheckBox chek[]
				 ,JComboBox<String> sexe ,JComboBox<String> nation, JLabel vide1,JTextField semsPTxt) {
		for(int i=0;i<tabField.length;i++) {
			tabField[i].setText("");
		}
			num.setText("");
...
public static void desable1(JTextField txtTab1[],JTextField numBaccTxt,JTextField numTelTxt,JTextField dateNaisTxt
	...
	public static void sauvegarde() throws IOException {
		 ...
		 XSSFSheet spreadsheet3=workbook.createSheet("Inscriptions");
		 XSSFSheet spreadsheet=workbook.createSheet("Donnée");
		 XSSFSheet spreadsheet1=workbook.createSheet("Semestre");
		 XSSFSheet spreadsheet2=workbook.createSheet("Validation");
		 XSSFRow row; 
		 XSSFRow row1;
		 XSSFRow row2;
		 XSSFRow row3;
		 
		 Map<String ,Object[]> data=DataBase.sauvegarde();
		 Set<String> keyId=data.keySet();
		 int rowId=0;
		 
		 Map<String ,Object[]> data1=DataBase.sauvegarde_sems();
		 Set<String> keyId1=data1.keySet();
		 int rowId1=0;
		 
		 Map<String ,Object[]> data2=DataBase.sauvegarde_validations();
		 Set<String> keyId2=data2.keySet();
		 int rowId2=0;
		 
		 Map<String ,Object[]> data3=DataBase.sauvegardeInsc();
		 Set<String> keyId3=data3.keySet();
		 int rowId3=0;
		 ...
		 for(String key:keyId1) {
			 ...
			 Object[] objectArr= data1.get(key);
			 int cellId=0;
		 ...
		 for(String key:keyId2) {
			 ...
			 Object[] objectArr= data2.get(key);
			 int cellId=0;
		 ...
		 for(String key:keyId3){
			 ...
			 Object[] objectArr= data3.get(key);
			 int cellId=0;
		 ...
		 String line = null;
		 try {
			 line=Files.readAllLines(Paths.get("C:\\Program Files (x86)\\JDev\\DataSciences\\package\\adress.txt")).get(2);
		 ...
		 File chemin=new File(line+"maths_"+String.valueOf(LoginPrincipale.typeAn.getSelectedItem()).substring(0,4)+"_"+
					String.valueOf(LoginPrincipale.typeAn.getSelectedItem()).substring(5,9));
		 chemin.mkdir();
		 File file =new File(chemin+"\\maths.xlsx");
		 if(!file.exists()) {
			 try {
				 file.createNewFile();
				...
				e.printStackTrace();
				JOptionPane.showMessageDialog(Main.main,"Erreur de connexion!!!" , "",
						JOptionPane.ERROR_MESSAGE);
			}
		 }
		 if(file.exists()) {
	 ...
	 @SuppressWarnings("resource")
	public static void sauvegarde_note(String mention, int i, int k,String type,int debutSems,int nbrSems) throws IOException {
		  
		 XSSFWorkbook workbook = new XSSFWorkbook();
		 XSSFSheet spreadsheet[] = new XSSFSheet[nbrSems];
		 for(int j=0;j<nbrSems;j++) {
			 spreadsheet[j]=workbook.createSheet("S"+(j+debutSems+1));
			 }
		 
		 HashMap<String,Map<String ,Object[]>> data=new HashMap<String,Map<String,Object[]>>();
		 HashMap<String, Set<String>> keyId= new HashMap<String, Set<String>>();
		 String semestre[]=new String[nbrSems];
		 HashMap<String, XSSFRow> row=new HashMap<String, XSSFRow>();
		 Integer[] rowId=new Integer[nbrSems];
		 
		 for(int j=0;j<nbrSems;j++) {
		  semestre[j]="Semestre"+(j+debutSems+1);
		  rowId[j]=0;
		  data.put("S"+(j+debutSems+1),DataBase.sauvegarde_note(mention, (j+debutSems+1), k,type,semestre[j]));
		  keyId.put("S"+(j+debutSems+1),data.get("S"+(j+debutSems+1)).keySet());
		 
		 for(String key:keyId.get("S"+(j+debutSems+1))){
			 row.put("S"+(j+debutSems+1),spreadsheet[j].createRow(rowId[j]++));
			 Object[] objectArr= data.get("S"+(j+debutSems+1)).get(key);
			 int cellId=0;
			 for(Object obj:objectArr) {
				 Cell cell=row.get("S"+(j+debutSems+1)).createCell(cellId++);
		 ...
		 File chemin=new File(line+"note_"+String.valueOf(LoginPrincipale.typeAn.getSelectedItem()).substring(0,4)+"_"+
					String.valueOf(LoginPrincipale.typeAn.getSelectedItem()).substring(5,9));
		 chemin.mkdir();
		 File file =new File(chemin+"\\note_"+DataBase.parcMaths[k]+"_"+type+".xlsx");
		 if(!file.exists()) {
			 try {
				 file.createNewFile();
	 ...
	 public static void sauvegarde_matiere(String mention, int k,int debutSems,int nbrSems) throws IOException {
		 @SuppressWarnings("resource")
		 XSSFWorkbook workbook = new XSSFWorkbook();
		 XSSFSheet spreadsheet[] = new XSSFSheet[nbrSems];
		 for(int j=0;j<nbrSems;j++) {
		 ...
		 for(int j=0;j<nbrSems;j++) {
		  semestre[j]="Semestre"+(j+debutSems+1);
		  rowId[j]=0;
		  data.put("S"+(j+debutSems+1),DataBase.sauvegarde_matier(mention, (j+debutSems+1), k,semestre[j]));
		  keyId.put("S"+(j+debutSems+1),data.get("S"+(j+debutSems+1)).keySet());
		 ...
		 File chemin=new File(line+"matiere_"+String.valueOf(LoginPrincipale.typeAn.getSelectedItem()).substring(0,4)+"_"+
					String.valueOf(LoginPrincipale.typeAn.getSelectedItem()).substring(5,9));
		 chemin.mkdir();
		 File file =new File(chemin+"\\matier_"+DataBase.parcMaths[k]+".xlsx");
		 if(!file.exists()) {
			 try {
				 file.createNewFile();
	 ...
	 public static boolean testMontant(String montant) {
		 try {
