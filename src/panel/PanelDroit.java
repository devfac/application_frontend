...
import impression.FenetreImpressionListeDroit;
...
public class PanelDroit extends JPanel{
	...
	public PanelDroit(String titre,JTable table,JButton bout,DefaultTableModel model) {
		this.titre=titre;
		this.table=table;
		this.bout=bout;
		this.model=model;
		...
		for (int c=0; c<5;c++) 
			...
			table.setPreferredScrollableViewportSize(new Dimension(610,520));
		...
		title=new JLabel ("DROITS DES ETUDIANTS VERSER EN "+titre);
		...
		bout1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				new FenetreImpressionListeDroit(model, titre).setVisible(true);
