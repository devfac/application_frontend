...
public class FenetreRelever extends JFrame{
	...
	public  FenetreRelever(JFrame parent, String title, boolean modal,String mention
		...
		for(int i=0;i<nbrSems;i++) {
			...
			titre3[i]=new JLabel("<html> <div id='bloc_page'>"
	                 ...
	                 + "&nbsp;&nbsp;&nbsp;&nbsp;/"+Main.date2textField.getText().substring(Main.date2textField.getText().lastIndexOf(" ")
	                		 ,Main.date2textField.getText().length())+"/UF/FAC.S/S.SCO </b> </font></p>"
	                 + "</font>"
	                 + " </div></html>");
	...
	public double calculeMoyenne(JTable table) {
		...
		for(int h=0;h<table.getRowCount();h++) {
			try {
				String value=(String)table.getValueAt(h, 2);
				if(value.equals("Abs")) {
					value="0.0";
				}
				S+=Double.parseDouble((String)table.getValueAt(h, 1))*Double.parseDouble(value);
			}catch(Exception e) {
	...
	public String type(JTable table,int ligne) {
		String S="";
		String value=(String)table.getValueAt(ligne, 2);
		if(!value.equals("Abs")) {
	...
	public String type1(JTable table) {
		...
		for(int h=0;h<table.getRowCount();h++) {
			String value=(String)table.getValueAt(h, 2);
			if(value.equals("Abs")) {
				S="<html><p STYLE='padding: 0 0 0 50px;'><u>Décision&nbsp;du&nbsp;jury</u>:&nbsp;Etudiant(e)&nbsp;qui&nbsp;n&nbsp;'a&nbsp;pas"
						+ "&nbsp;validé(e)&nbsp;"
						+ "les&nbsp;30&nbsp;crédits</html>";
			}else
			if(Double.parseDouble(value)<10) {
