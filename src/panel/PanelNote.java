...
public class PanelNote extends JTabbedPane {
	...
	public PanelNote(String mention,int nbrSems,int t,int debutSems,String type,String sems[],
		...
		for(int i=0;i<nbrSems;i++) {
			...
			liste[i]=new JButton(LoadImage.transformeb(130, 25, "/liste.jpg"));
			listeGrp[i]=new JButton(LoadImage.transformeb(130, 25, "/boutonGroupe.jpg"));
			...
			liste[i].setPreferredSize(new Dimension(130,25));
			listeGrp[i].setPreferredSize(new Dimension(130,25));
			...
			if(type=="normal" || type=="rattrapage") {
				...
				listeGrp[i].setVisible(true);
			}else {
				releve[i].setVisible(true);
				fen[i].setVisible(true);
				res[i].setVisible(true);
				liste[i].setVisible(false);
				listeGrp[i].setVisible(false);
			}
		...
		listeGrp[j].addActionListener(new ListeGrp());
	}
		}
	...
	public class ListeGrp implements ActionListener{ 
		public void actionPerformed(ActionEvent arg0) {
			Main.main.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
			new FenetreListeParGroupe(Main.main," RESULTAT ", true,mention,t,type,parcours,nbrSems,debutSems,sems,post).setVisible(true);
			Main.main.setCursor(Cursor.getPredefinedCursor(Cursor.DEFAULT_CURSOR));
		}
